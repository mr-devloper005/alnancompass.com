...
const inputClass = 'h-12 rounded-xl border border-slate-200 bg-white px-4 text-base font-bold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-[#5b19ff] focus:ring-2 focus:ring-[#5b19ff]/20'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-xl bg-[#5b19ff] px-6 text-sm font-black uppercase tracking-[0.18em] text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#6d35ff] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      <label className={labelClass}>Email address<input className={inputClass} type="email" placeholder="you@example.com" value={email} onChange={(event) => setEmail(event.target.value)} required /></label>
...
export function EditableLocalSignupForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      <label className={labelClass}>Full name<input className={inputClass} placeholder="Business owner or manager" value={name} onChange={(event) => setName(event.target.value)} required /></label>
      <label className={labelClass}>Email address<input className={inputClass} type="email" placeholder="you@example.com" value={email} onChange={(event) => setEmail(event.target.value)} required /></label>
      <label className={labelClass}>Password<input className={inputClass} type="password" placeholder="Minimum 4 characters" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
